...
def mask_dropout_keypoints(
    ...
    """
    # Optimize the logic to reduce redundant array operations
    y_coords = keypoints[:, 1].astype(int)
    x_coords = keypoints[:, 0].astype(int)
    all_dropped = dropout_mask[y_coords, x_coords].all(axis=-1) if dropout_mask.ndim == 3 else dropout_mask[y_coords, x_coords]
    keep_indices = np.logical_not(all_dropped)
